export default {
  async fetch(request, env) {
    ...
    if (path === "/" || path === "") {
      return json({ ok: true, message: "One and Done API is active." });
    }
    ...
    if (path === "/submit" && request.method === "POST") {
      ...
      await env.PICKS.put(`pick:${name.toLowerCase()}`, JSON.stringify(entry));
    ...
    if (path === "/admin" && request.method === "POST") {
      ...
      if (!env.ADMIN_KEY || adminKey !== env.ADMIN_KEY) return json({ ok: false, message: "Unauthorized." }, 401);
      ...
      if (action === "lock") s.locked = true;
        ...
        s.locked = false;    // FORCE RESET
        s.revealed = false;  // FORCE RESET
      } else {
